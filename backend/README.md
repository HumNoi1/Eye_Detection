...
- ⏱️ **5-second sliding window** สำหรับ prediction ที่แม่นยำ
...
- 🎚️ **Adjustable confidence threshold** - ปรับความแม่นยำได้แบบ real-time
...
### Configuration
- `GET /config/confidence` - ดู confidence threshold ปัจจุบัน
- `POST /config/confidence?confidence={value}` - ตั้งค่า confidence threshold (0.0-1.0)
...
ค่า confidence threshold กำหนดระดับความมั่นใจขั้นต่ำในการตรวจจับ:

- **ค่าเริ่มต้น**: 0.25 (25%)
- **ช่วงค่า**: 0.0 - 1.0
- **แก้ไขได้**: ผ่าน API หรือแก้ไขในโค้ด

```python
# ใน main.py (บรรทัดที่ 43)
CONFIDENCE_THRESHOLD = 0.25  # แก้ไขค่านี้ได้ตามต้องการ
```

**คำแนะนำ:**
- **0.1-0.3**: ตรวจจับได้ง่าย, เหมาะกับสภาพแสงน้อย (อาจมี false positive)
- **0.25-0.4**: สมดุลระหว่างความแม่นยำและการตรวจจับ ✅ แนะนำ
- **0.5-0.7**: แม่นยำสูง, เหมาะกับสภาพแสงดี (อาจพลาดบางกรณี)
- **0.8-1.0**: แม่นยำมาก, แต่ตรวจจับได้ยาก

### ปรับค่าแบบ Real-time
